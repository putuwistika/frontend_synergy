...
> & {
  ...
  /** Aliases (so usages with startIcon/endIcon won’t leak to DOM) */
  startIcon?: React.ReactNode; // alias of leftIcon
  endIcon?: React.ReactNode;   // alias of rightIcon
...
export const Input = React.forwardRef<HTMLInputElement, InputProps>(function Input(
  {
    ...
    size = "md",
    fullWidth = true,
    className,        // <-- akan dipakai di <input>
    wrapperClassName, // <-- untuk wrapper luar
    type = "text",
    disabled,
    required,
    ...rest // <- TIDAK akan berisi startIcon/endIcon karena sudah di-destructure
  },
  ref
) {
  ...
  // gunakan useId SELALU (tidak kondisional)
  const autoId = React.useId();
  const inputId = id ?? autoId;

  // pakai alias bila left/rightIcon tidak disuplai
  const left = leftIcon ?? startIcon;
  const right = rightIcon ?? endIcon;

  const hasLeft = Boolean(left || prefix);
  const hasRight = Boolean(
    right || suffix || (onClear && rest.value) || (isPassword && showPasswordToggle)
  );
  ...
  const buttonAdornment = "absolute inset-y-0 right-2 flex items-center gap-1";
  ...
  return (
    <div className={cn(fullWidth && "w-full", wrapperClassName)}>
      ...
      <div className={cn("relative", disabled && "opacity-70 cursor-not-allowed")}>
        {/* Left icon */}
        {left && (
        ...
        {/* Prefix */}
        {prefix && (
          <span className="absolute inset-y-0 left-9 flex items-center border-r border-white/10 px-2 text-sm text-white/70">
            {prefix}
          </span>
        )}
        ...
        <input
          ...
          className={cn(
            ...
            className // <-- pakai className milik user
          )}
          disabled={disabled}
          required={required}
          {...rest}
        ...
        <div className={buttonAdornment}>
              ...
              {showPassword ? (
                ...
                <svg className="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                  <path
                    ...
                    d="M3 3l18 18M10.585 10.585A3 3 0 0012 15a3 3 0 002.121-.879M9.88 9.88C9.323 10.437 9 11.18 9 12a3 3 0 003 3c.82 0 1.563-.323 2.12-.88M6.343 6.343C4.253 7.82 2.82 9.72 2 12c1.5 4 6 7 10 7 1.632 0 3.184-.37 4.586-1.03M17.657 17.657C19.747 16.18 21.18 14.28 22 12c-1.5-4-6-7-10-7-1.05 0-2.067.145-3.03.415"
                  />
                </svg>
              ) : (
                // Eye
          ...
          {right && <span className="pointer-events-none ml-1 text-white/70">{right}</span>}
        </div>
      </div>
